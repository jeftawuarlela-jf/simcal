...
# ─────────────────────────────────────────────────────────────
# Config writer — writes a real config.py so simulationwithcalendar.py
...
if run_clicked:
    ...
    # Write config.py and copy simulationwithcalendar.py into work_dir
    write_config(work_dir, out_dir, csv_path)

    # Find simulationwithcalendar.py — either next to this app or uploaded earlier
    sim_src = os.path.join(os.path.dirname(__file__), "simulationwithcalendar.py")
    if not os.path.exists(sim_src):
        st.error("simulationwithcalendar.py not found next to app2_plotly.py. Please place both files in the same folder.")
        st.stop()

    import shutil
    shutil.copy(sim_src, os.path.join(work_dir, "simulationwithcalendar.py"))
    ...
    proc = subprocess.Popen(
        [sys.executable, "simulationwithcalendar.py"],
        cwd=work_dir,
